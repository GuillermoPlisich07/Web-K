...
import { getTeamDashboard, type TeamDashboard } from "../../services/dashboardApi";
import { COLORS, GRAD } from "../../lib/theme";
import { formatScore, formatPercent } from "../../lib/dashboardFormat";
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <h2 className="text-xl font-bold text-white tracking-tight">Dashboard Ejecutivo</h2>
          <span
            className="flex items-center gap-1 text-[10px] uppercase tracking-widest font-semibold px-2 py-1 rounded-full"
            style={{ backgroundColor: COLORS.hover, color: COLORS.textMuted, border: `1px solid ${COLORS.border}` }}
          >
            <Eye size={10} /> Solo lectura
          </span>
        </div>
        <p className="text-sm" style={{ color: COLORS.textMuted }}>Visibilidad completa de la actividad de la empresa</p>
      </div>
      ...
      {!hasTeamActivity ? (
        <DashboardCard>
          <DashboardEmptyState
            ...
            description="Las métricas de la empresa van a aparecer acá una vez que los vendedores empiecen a practicar."
          />
        </DashboardCard>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard label="Sesiones (6 sem.)" value={String(totalSessions)} icon={TrendingUp} gradientValue />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <DashboardCard className="lg:col-span-2">
              <div className="text-sm font-semibold text-white mb-4">Tendencia de Actividad</div>
              {team.activityTrend.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={team.activityTrend}>
